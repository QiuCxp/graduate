...
if __name__ == "__main__":
    ...
    # shared vars (declare here so finally-block can reference safely)
    ...
    # EE shared buffers (created depending on args.ee)
    ...
    try:
        ...
        if args.ee == "dex3":
            ...
            # ✅ NEW: controller trigger values for dex3
            left_trigger_value = Value("d", 0.0, lock=True)  # [input]
            right_trigger_value = Value("d", 0.0, lock=True)  # [input]

            dual_hand_data_lock = Lock()
            dual_hand_state_array = Array("d", 14, lock=False)  # [output] left(7)+right(7) state
            dual_hand_action_array = Array("d", 14, lock=False)  # [output] left(7)+right(7) action

            # ✅ IMPORTANT: Dex3_1_Controller must accept (left_trigger_value, right_trigger_value) in its __init__
            ...
            right_hand_pos_array = Array("d", 75, lock=True)  # [input]
            dual_hand_data_lock = Lock()
            dual_hand_state_array = Array("d", 12, lock=False)  # [output]
            dual_hand_action_array = Array("d", 12, lock=False)  # [output]
        ...
        while not STOP:
            ...
            if (args.ee in ["dex3", "inspire_dfx", "inspire_ftp", "brainco"]) and args.input_mode == "hand":
                ...
                # (optional) if dex3, also allow trigger override even in hand mode
                if args.ee == "dex3":
                    left_trigger_norm = (10.0 - tele_data.left_ctrl_triggerValue) / 10.0
                    right_trigger_norm = (10.0 - tele_data.right_ctrl_triggerValue) / 10.0
                    if tele_data.left_ctrl_trigger and left_trigger_norm <= 0.001:
                        left_trigger_norm = 1.0
                    if tele_data.right_ctrl_trigger and right_trigger_norm <= 0.001:
                        right_trigger_norm = 1.0
                    left_trigger_norm = max(0.0, min(1.0, left_trigger_norm))
                    right_trigger_norm = max(0.0, min(1.0, right_trigger_norm))
                    with left_trigger_value.get_lock():
                        left_trigger_value.value = left_trigger_norm
                    with right_trigger_value.get_lock():
                        right_trigger_value.value = right_trigger_norm

            # ✅ NEW: dex3 + controller -> feed trigger values
                ...
                with left_gripper_value.get_lock():
                    left_gripper_value.value = left_trigger_norm
                with right_gripper_value.get_lock():
                    right_gripper_value.value = right_trigger_norm
            ...
            if args.record:
                ...
                # ee state/action
                if args.ee == "dex3" and args.input_mode in ["hand", "controller"]:
                ...
                if RECORD_RUNNING:
                    ...
                    if camera_config["head_camera"]["binocular"]:
                        if head_img is not None:
                            colors["color_0"] = head_img[:, : camera_config["head_camera"]["image_shape"][1] // 2]
                            colors["color_1"] = head_img[:, camera_config["head_camera"]["image_shape"][1] // 2 :]
                        else:
            ...
            sleep_time = max(0.0, (1.0 / args.frequency) - time_elapsed)
